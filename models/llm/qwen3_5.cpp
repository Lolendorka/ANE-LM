...
Qwen35Args Qwen35Args::from_json(const json& j) {
    ...
    if (tc.contains("layer_types")) {
        for (auto& lt : tc["layer_types"]) {
            ...
            args.layer_types.push_back(s == "linear_attention" ? LayerType::LinearAttention : LayerType::FullAttention);
        }
    } else {
        for (int i = 0; i < args.num_hidden_layers; i++)
            args.layer_types.push_back((i + 1) % args.full_attention_interval == 0 ? LayerType::FullAttention : LayerType::LinearAttention);
    }
    return args;
}
...
Qwen35Model::~Qwen35Model() {
    ...
    free(scratch_qkv_); free(scratch_conv_); free(scratch_y_);
    free(scratch_attn_); free(scratch_tmp_);
    ...
    for (int L = 0; L < num_layers_; L++) {
        ...
        if (lw.type == LayerType::LinearAttention) {
            free(lw.deltanet.in_proj_a); free(lw.deltanet.in_proj_b);
            free(lw.deltanet.conv1d_w); free(lw.deltanet.A);
            free(lw.deltanet.dt_bias); free(lw.deltanet.norm_w);
        } else { free(lw.full_attn.q_norm); free(lw.full_attn.k_norm); }
    }
    for (int L = 0; L < num_layers_; L++) {
        if (layer_types_[L] == LayerType::FullAttention) { free(kv_caches_[L].k_cache); free(kv_caches_[L].v_cache); }
        if (layer_types_[L] == LayerType::LinearAttention) { free(delta_states_[L].ssm_state); free(delta_states_[L].conv_state); }
...
void Qwen35Model::reset() {
    for (int L = 0; L < num_layers_; L++) {
        if (layer_types_[L] == LayerType::FullAttention) {
            kv_caches_[L].len = 0; kv_caches_[L].start = 0;
...
void Qwen35Model::apply_args(const Qwen35Args& args) {
    ...
    head_dim_ = args.head_dim; rot_dim_ = args.rotation_dim();
    rope_theta_ = args.rope_theta; rms_eps_ = args.rms_norm_eps;
    lin_num_heads_ = args.linear_num_key_heads; lin_num_val_heads_ = args.linear_num_value_heads;
    lin_key_dim_ = args.linear_key_head_dim; lin_val_dim_ = args.linear_value_head_dim;
    lin_total_key_ = lin_num_heads_ * lin_key_dim_; lin_total_val_ = lin_num_val_heads_ * lin_val_dim_;
...
bool Qwen35Model::load(const std::string& model_dir) {
    ...
    if (!sf) { fprintf(stderr, "Failed to open model weights\n"); return false; }
    const SFTensor* embed = sf->find("model.language_model.embed_tokens.weight");
    if (!embed || embed->ndims != 2) { fprintf(stderr, "Missing embed_tokens\n"); return false; }
    const SFTensor* gate = sf->find("model.language_model.layers.0.mlp.gate_proj.weight");
    if (!gate || gate->ndims != 2) { fprintf(stderr, "Missing gate_proj\n"); return false; }
    ...
    layers_.resize(num_layers_); delta_states_.resize(num_layers_);
    kv_caches_.resize(num_layers_); ane_layers_.resize(num_layers_);
    for (int L = 0; L < num_layers_; L++) {
        if (layer_types_[L] == LayerType::FullAttention) {
...
bool Qwen35Model::load_weights(ModelWeights* sf) {
    ...
    for (int L = 0; L < num_layers_; L++) {
        auto& lw = layers_[L]; lw.type = layer_types_[L];
        snprintf(name, sizeof(name), "model.language_model.layers.%d.input_layernorm.weight", L);
        lw.input_layernorm = sf->load_norm_weight(name, hidden_size_); if (!lw.input_layernorm) return false;
        snprintf(name, sizeof(name), "model.language_model.layers.%d.post_attention_layernorm.weight", L);
        lw.post_attention_layernorm = sf->load_norm_weight(name, hidden_size_); if (!lw.post_attention_layernorm) return false;
        if (lw.type == LayerType::LinearAttention) {
            auto& dw = lw.deltanet;
            ...
            if (dw.A) for (int i = 0; i < lin_num_val_heads_; i++) dw.A[i] = expf(dw.A[i]);
            ...
            if (!dw.in_proj_a || !dw.in_proj_b || !dw.conv1d_w || !dw.A || !dw.dt_bias || !dw.norm_w) {
            ...
            if (!fw.q_norm || !fw.k_norm) { fprintf(stderr, "Failed to load FullAttn weights for layer %d\n", L); return false; }
        }
    }
...
bool Qwen35Model::compile_ane(ModelWeights* sf, const std::string& blob_dir) {
    ...
    for (int L = 0; L < num_layers_; L++) {
        ...
        if (layer_types_[L] == LayerType::LinearAttention) {
            ...
            if (use_blobs) ane_layers_[L].first_proj = ane_compile_fused_2_blob(blob_path(blob_dir, name), lin_qkv_dim_, blob_path(blob_dir, name2), lin_total_val_, hidden_size_);
            else           ane_layers_[L].first_proj = ane_compile_fused_2(sf->get_bf16_ptr(name), lin_qkv_dim_, sf->get_bf16_ptr(name2), lin_total_val_, hidden_size_);
        } else {
            ...
            if (use_blobs) ane_layers_[L].first_proj = ane_compile_fused_3_blob(blob_path(blob_dir, name), full_q_dim_, blob_path(blob_dir, name2), full_kv_dim_, blob_path(blob_dir, name3), full_kv_dim_, hidden_size_);
            else           ane_layers_[L].first_proj = ane_compile_fused_3(sf->get_bf16_ptr(name), full_q_dim_, sf->get_bf16_ptr(name2), full_kv_dim_, sf->get_bf16_ptr(name3), full_kv_dim_, hidden_size_);
        }
        if (!ane_layers_[L].first_proj) { fprintf(stderr, "ANE first_proj failed layer %d\n", L); return false; }
        int attn_dim;
        if (layer_types_[L] == LayerType::LinearAttention) { snprintf(name, sizeof(name), "model.language_model.layers.%d.linear_attn.out_proj.weight", L); attn_dim = lin_total_val_; }
        else { snprintf(name, sizeof(name), "model.language_model.layers.%d.self_attn.o_proj.weight", L); attn_dim = full_out_dim_; }
        if (use_blobs) ane_layers_[L].o_proj = ane_compile_matmul_blob(blob_path(blob_dir, name), hidden_size_, attn_dim);
        else           ane_layers_[L].o_proj = ane_compile_matmul(sf->get_bf16_ptr(name), hidden_size_, attn_dim);
        ...
        if (use_blobs) ane_layers_[L].fused_ffn = ane_compile_fused_ffn_blob(blob_path(blob_dir, name), blob_path(blob_dir, name2), blob_path(blob_dir, name3), hidden_size_, intermediate_size_);
        else           ane_layers_[L].fused_ffn = ane_compile_fused_ffn(sf->get_bf16_ptr(name), sf->get_bf16_ptr(name2), sf->get_bf16_ptr(name3), hidden_size_, intermediate_size_);
    ...
    LOG("  %d ANE kernels ready (compiled=%d, cached=%d)\n", compiled+cached, compiled, cached);
    if (!compile_lm_head_ane(sf, blob_dir)) LOG("ANE LM head disabled\n");
...
bool Qwen35Model::compile_lm_head_ane(ModelWeights* sf, const std::string& blob_dir) {
    ...
    if (!use_blobs) {
        ...
        if (!embed_bf16) { fprintf(stderr, "ANE LM head: missing embed_tokens BF16\n"); return false; }
    }
    int chunk = lm_head_chunk_; if (chunk > vocab_size_) chunk = vocab_size_;
    int chunks = (vocab_size_ + chunk - 1) / chunk;
    lm_head_kernels_.resize(chunks, nullptr);
    LOG("  LM head ANE: compiling %d chunks\n", chunks);
    for (int c = 0; c < chunks; c++) {
        int offset = c * chunk, rows = vocab_size_ - offset; if (rows > chunk) rows = chunk;
        if (use_blobs) { embed_bf16 = sf->get_bf16_ptr("model.language_model.embed_tokens.weight"); if (!embed_bf16) return false; }
        lm_head_kernels_[c] = ane_compile_matmul(embed_bf16 + (int64_t)offset * hidden_size_, rows, hidden_size_);
        if (!lm_head_kernels_[c]) { fprintf(stderr, "ANE LM head compile failed chunk %d\n", c); free_lm_head_ane(); return false; }
    }
...
bool Qwen35Model::forward_deltanet_core(int L, float* x, float* pre_oproj) {
    auto& dw = layers_[L].deltanet; auto& st = delta_states_[L];
    float* qkv_z = scratch_qkv_;
    if (!ane_matvec(ane_layers_[L].first_proj, qkv_z, x, hidden_size_, lin_qkv_dim_ + lin_total_val_)) {
        fprintf(stderr, "ANE first_proj failed layer %d (DeltaNet)\n", L); return false;
    }
    float* mixed_qkv = qkv_z, *z = qkv_z + lin_qkv_dim_;
    float* a_vec = scratch_tmp_, *b_vec = scratch_tmp_ + lin_num_val_heads_;
    ...
    float* Q = conv_out, *K = conv_out + lin_total_key_, *V = conv_out + lin_total_key_ * 2;
    float* y = scratch_y_;
    ...
    for (int kh = 0; kh < lin_num_heads_; kh++) {
        float* qh = Q + kh * lin_key_dim_, *kh_ptr = K + kh * lin_key_dim_;
        l2_normalize(qh, lin_key_dim_); l2_normalize(kh_ptr, lin_key_dim_);
        float qs = q_scale; vDSP_vsmul(qh, 1, &qs, qh, 1, (vDSP_Length)lin_key_dim_);
        for (int vsub = 0; vsub < val_heads_per_key; vsub++) {
            int vh = kh * val_heads_per_key + vsub;
            float* vh_ptr = V + vh * lin_val_dim_, *yh = y + vh * lin_val_dim_;
    ...
    for (int h = 0; h < lin_num_val_heads_; h++)
        rmsnorm_gated(pre_oproj + h * lin_val_dim_, y + h * lin_val_dim_, z + h * lin_val_dim_, dw.norm_w, lin_val_dim_);
    return true;
}

bool Qwen35Model::forward_full_attn_core(int L, float* x, float* pre_oproj, int pos) {
    auto& fw = layers_[L].full_attn; auto& cache = kv_caches_[L];
    float* qkv_buf = scratch_qkv_;
    if (!ane_matvec(ane_layers_[L].first_proj, qkv_buf, x, hidden_size_, full_q_dim_ + full_kv_dim_ * 2)) {
        fprintf(stderr, "ANE first_proj failed layer %d (FullAttn)\n", L); return false;
    }
    float* q_gate_raw = qkv_buf, *k_raw = qkv_buf + full_q_dim_, *v_raw = qkv_buf + full_q_dim_ + full_kv_dim_;
    int nqh = num_q_heads_, nkh = num_kv_heads_;
    float eps = rms_eps_; float* q_n = fw.q_norm; float* k_n = fw.k_norm; int hd = head_dim_;
    dispatch_apply(nqh + nkh, dispatch_get_global_queue(QOS_CLASS_USER_INTERACTIVE, 0),
        ^(size_t h) {
            if (h < (size_t)nqh) { float* qh = q_gate_raw + h * hd * 2; rmsnorm(qh, qh, q_n, hd, eps); }
    ...
    apply_rope_cached(q_gate_raw, k_raw, num_q_heads_, num_kv_heads_,
                      head_dim_, head_dim_ * 2, head_dim_, rot_dim_, pos, rope_theta_, cos_row, sin_row);
    int slot;
    if (cache.len < cache.capacity) {
        slot = cache.start + cache.len; if (slot >= cache.capacity) slot -= cache.capacity; cache.len++;
    } else { slot = cache.start++; if (cache.start >= cache.capacity) cache.start = 0; }
...
// ============ PERF: forward_layers -- shared by forward() and prefill_step() ============
bool Qwen35Model::forward_layers(int token_id, int pos) {
    ...
    for (int L = 0; L < num_layers_; L++) {
        ...
        if (layer_types_[L] == LayerType::LinearAttention) {
            if (!forward_deltanet_core(L, x_norm_, pre_oproj)) return false;
        } else {
        ...
        if (!ane_matvec(ane_layers_[L].o_proj, attn_out, pre_oproj, attn_dim, hidden_size_)) {
            fprintf(stderr, "ANE o_proj failed layer %d\n", L); return false;
        }
        ...
        if (!ane_matvec(ane_layers_[L].fused_ffn, mlp_out, x_norm_, hidden_size_, hidden_size_)) {
            fprintf(stderr, "ANE fused_ffn failed layer %d\n", L); return false;
        }
...
// ============ PERF: prefill_step -- skips LM head (~10 ANE dispatches saved per token) ============
...
float* Qwen35Model::forward(int token, int pos) {
    if (!forward_layers(token, pos)) return nullptr;

    // ============ PATCH 4: Parallel LM head via dispatch_apply ============
    if (ane_lm_head_enabled_ && !lm_head_kernels_.empty()) {
        ...
        if (!lm_ok.load()) { free_lm_head_ane(); matvec(logits_, embed_tokens_, x_, vocab_size_, hidden_size_); }
    } else {
